...
# Build docker images
ENABLE_MULTIARCH_IMAGES="${ENABLE_MULTIARCH_IMAGES}" sh build_images.sh "${VERSION}" "${PREFIX}"
...
if [[ "${ENABLE_MULTIARCH_IMAGES}" == "false" ]]; then
  ...
  do
    IMAGES+=$(docker images -f reference="${PREFIX}/springcloud-bookinfo*:$v" --format "{{.Repository}}:$v")
    IMAGES+=" "
  done

  # Check that $IMAGES contains the images we've just built
  if [[ "${IMAGES}" =~ ^\ +$ ]] ; then
    echo "Found no images matching prefix \"${PREFIX}/springcloud-bookinfo\"."
...
do
  ...
  # $IMAGE has the following format: istio/springcloud-bookinfo*:"$v".
  # We want to get the sample app name from $IMAGE (the springcloud-bookinfo* portion)
  # to create the file to store the results of the scan for that image. The first
  # part of the $IMAGE_NAME gets springcloud-bookinfo*:"$v", and the second part gets
  # 'springcloud-bookinfo*'.
...
find ./platform -name "*bookinfo*.yaml" -exec sed -i.bak "s#image:.*\\(\\/springcloud-bookinfo-.*\\):.*#image: ${PREFIX//\//\\/}\\1:$VERSION#g" {} +
